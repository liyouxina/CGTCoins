...
class WalletController extends Controller
{
    public function getSetting(){
        $kefu=Setting::getValueByKey('ADMIN_KEFU');
        return $this->success($kefu);
    }
    //我的资产
...
//资产划转
    ...
    public function getWalletAddressIn()
    {
		...
		if($currency_id==3){
			$address = ['omni'=>$currencyInfo->address_omni,'erc20' => $currencyInfo->address_erc,'trc20' => $currencyInfo->address_trc];
		}else{
...
//                ->delay(Carbon::now()->addMinutes(1));
    ...
    public function flashAgainstList(Request $request)
    {
        ...
        foreach ($left as $k => $v) {
            ...
            $nowPrice= CurrencyQuotation::where('currency_id',$v['id'])->first()->now_price??'';
        ...
        $right = Currency::where('is_match', 1)->get();
    ...
    public function flashAgainst(Request $request)
    {
        try {
            ...
            $result2=change_wallet_balance($r_wallet,2,$absolute_quantity,AccountLog::DEBIT_BALANCE_ADD,'闪兑通过,增加余额');
            if($result1 !== true){
