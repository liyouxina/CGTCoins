...
class UserController extends Controller
{
    ...
    // public function ztpayLogView(){
    //     $currencies = Currency::where('id','<>',34)->get();
    //     return view('admin.wallet.ztpay', ['currencies' => $currencies]);
    // }
    
    // public function ztpayLog(Request $request){
    ...
    //     }
    //     $list = AccountLog::where('type',118)->where('info','充币记录')->where($where)->where('currency','<>',34)->orderBy('id','desc')->paginate($limit);
    //     $items = $list->getCollection();
    //     // echo 123;exit;
    //     $items->transform(function ($item, $key) {
    //         $userWallet = UsersWallet::where('currency',$item->currency)->where('user_id',$item->user_id)->first();
    //         if($userWallet){
    //             $item->address = $userWallet->address;
    //             $item->address_2 = $userWallet->address_2;
    //             $item->address_3 = $userWallet->address_3;
    //         }
    //         return $item;
    //     });
...
//            var_dump($currency->type);var_dump($total_account);var_dump($origin_number);var_dump($user_address);var_dump($btc_key);var_dump($fee);die;
    ...
    public function ztpayLog(Request $request){
        ...
        $where = [];
        if($currency){
            $where['currency.id'] = $currency;
        }
        $list = DB::table('ztpay_log')->join('users_wallet','wallet_id','=','users_wallet.id')
            ->join('currency','currency.id','=','users_wallet.currency')
            ->join('users','users.id','=','users_wallet.user_id')
            ->where($where)
            ->select(['ztpay_log.*','users.account_number','currency.name as currency_name'])
            ->orderBy('ztpay_log.id','desc')
            ->paginate($limit);
        return $this->layuiData($list);
    }
    
    public function addressPrivate(Request $request){
        $id = $request->get('id');
        
       $wallet = UsersWallet::find($id);
    //   echo $wallet->private;exit;
    //     var_dump($wallet->private);var_dump($id);exit;
        return view('admin.wallet.private', ['result' => $wallet]);
    }
