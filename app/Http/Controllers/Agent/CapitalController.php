...
class CapitalController extends Controller
{
    ...
    public function wallettotalList(Request $request)
    {
        ...
        $list = UsersWallet::join('currency','currency.id','=','currency')->where('user_id',$user_id)->select(['currency.name','users_wallet.*'])->paginate($limit);//Currency::orderBy('id', 'asc')->select(['id', 'name'])->paginate($limit);
        
        foreach ($list->items() as &$value) {
            $value->_ru = AccountLog::where('type', AccountLog::WALLET_CURRENCY_IN)
